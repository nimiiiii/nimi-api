...
import { IFileSchema, createFileEntry, getFileEntry, updateFileEntry } from "lib/database";

const compress = util.promisify(snappy.compress);
const uncompress = util.promisify(snappy.uncompress);

export type ResolverRegion =
    | "en"
    | "jp"
    | "cn"
    | "kr"
...
export default abstract class Resolver {
     ...
     */
    abstract resolve(model: IModel) : Promise<any[]>
    ...
    async get(file: string) : Promise<any> {
        ...
        const useMongoDB = process.env.NIMI_MONGODB_HOST && process.env.NODE_ENV !== "development";
        ...
        if (useMongoDB) {
            ...
            if (cached?.hash === remote.sha) {
                const raw = <string>(await uncompress(Buffer.from(cached.contents.buffer), { asBuffer: false }));
